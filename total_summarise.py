from transformers import PegasusForConditionalGeneration, PegasusTokenizer, GenerationConfig
import spacy
...
def keyword_sentence_similarity(sentence):
    ...
    relevant_keywords = [keywords[i] for i in range(len(similarities[0])) if similarities[0][i] > 0.2]
...
def transcripts_generate(filepath):
    ...
    with open("finetuned_ba_flanllama.txt","w") as file:
        file.write(text)
